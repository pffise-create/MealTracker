...
const directions = [
  { id: "01", name: "Electric Blue", short: "Crisp energy on white", className: "electric-blue", accent: "#125BFF", accentSoft: "#DDE8FF", bg: "#FFFFFF", ink: "#12243D", muted: "#66758F", line: "#D8E1F0", font: "Plus Jakarta Sans + DM Sans", icon: "Lucide", image: "User / generated photos", character: "Confident, bright, direct, and unmistakably digital.", tradeoff: "Creates the strongest action contrast, but blue needs to stay out of neutral macro status signals." },
];
...
export default function Home() {
  return <main className="phase2-board">
    <header className="phase2-header"><div className="phase2-brand"><img src="/manus-storage/mealtrack-mark_41ae0274.png" alt="" /><span>MEALTRACK<small>PHASE 02 / VISUAL EXPLORATION</small></span></div><div className="phase2-title"><span className="eyebrow">Selected direction · same approved UX</span><h1>Make the next log<br /><em>feel like you.</em></h1><p>A focused visual direction for an adult, motivating, iPhone-native meal tracker. It keeps the prediction, neutral macro snapshot, direct capture, and one clear Log action unchanged.</p></div><div className="phase2-license"><span>ASSET SOURCES</span><strong>Fonts: Google Fonts / OFL</strong><strong>Icons: open-source sets</strong><strong>Food: user, open, or generated</strong></div></header>
    <section className="direction-rail" aria-label="Selected Electric Blue visual direction">{directions.map((direction) => <DirectionCard key={direction.id} direction={direction} />)}</section>
    <footer className="phase2-footer"><span>WHY ELECTRIC BLUE</span><p>White space keeps the meal decision clear; electric blue makes the next action obvious without turning neutral nutrition data into a grade.</p></footer>
  </main>;
}
